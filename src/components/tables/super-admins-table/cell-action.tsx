...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const { mutate: deleteMutation, isPending: deleteIsPending } = useMutation({
    mutationFn: async () => {
      const res = await axiosInstance.delete(`/super-admins/${data.id}`);
      return res;
    },
    onSuccess: (res: any) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.SUPERADMIN, data.id],
      });
      if (res.success) {
        toast({
          variant: "default",
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/manage-admins/super-admins/${data.id}`)
            }
          >
